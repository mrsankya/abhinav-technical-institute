...
30. **Certificate Database Storage & Synchronization Fix (`functions/api/certificates/index.js`, `functions/api/certificates/[id].js`, `src/services/api.ts`, `src/components/AdminPanelModal.tsx`, `src/App.tsx`):**
    - **Root Cause:** In Cloudflare D1 `abhinav_db`, the `certificates` table schema defined `reg_number` as the primary key and had strict columns (`reg_number`, `student_name`, `course_name`, `grade`, `percentage`, `issue_date`, `valid_until`, `status`, `institute_center`, `remarks`, `raw_json`). However, `functions/api/certificates/index.js` was executing an INSERT referencing non-existent columns (`id`, `studentName`, `fatherName`, `course`, `isValid`), causing SQLite code 7500 / column errors. Additionally, CORS preflight was returning 405 because `onRequestOptions` was missing.
