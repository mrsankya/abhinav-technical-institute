const DEFAULT_GR_LIST = [
  ...
  },
  {
    ...
    badgeColor: 'bg-emerald-700 text-white',
    codeNumber: '२०१२०९२८१५०२१२१३०१',
  },
];

async function ensureGrTable(db) {
  if (!db) return;
  await db.prepare(`CREATE TABLE IF NOT EXISTS government_gr (
    id TEXT PRIMARY KEY,
...
function formatDbRowToGr(row) {
  ...
  return {
    ...
    titleEn: row.title_en || row.title_mr,
    number: row.number,
    date: row.date,
    deptMr: row.dept_mr,
    deptEn: row.dept_en,
    summaryMr: row.summary_mr || '',
    summaryEn: row.summary_en || '',
    pdfPath: row.pdf_path,
    status: row.status || 'GOVERNMENT RESOLUTION',
    badgeColor: row.badge_color || 'bg-[#002760] text-white',
...
export async function onRequestGet(context) {
  ...
  try {
    if (db) {
      await ensureGrTable(db);
      const { results } = await db.prepare('SELECT * FROM government_gr ORDER BY created_at DESC').all();
      if (results && results.length > 0) {
        const formatted = results.map(formatDbRowToGr);
  ...
  return new Response(JSON.stringify(DEFAULT_GR_LIST), {
    headers: { 'Content-Type': 'application/json', 'Access-Control-Allow-Origin': '*' },
  });
}
...
export async function onRequestPost(context) {
  ...
  // Check for /api/gr/reset
  if (url.pathname.endsWith('/reset')) {
    try {
      if (db) {
        await ensureGrTable(db);
        await db.prepare('DELETE FROM government_gr').run();
      }
    } catch (e) {}
    return new Response(JSON.stringify(DEFAULT_GR_LIST), {
      headers: { 'Content-Type': 'application/json', 'Access-Control-Allow-Origin': '*' },
  ...
  try {
    ...
    if (!gr || !gr.titleMr || !gr.number) {
      return new Response(JSON.stringify({ error: 'Title in Marathi and GR Number are required' }), {
        status: 400,
        headers: { 'Content-Type': 'application/json', 'Access-Control-Allow-Origin': '*' },
      });
    }

    const id = gr.id || `gr-${Date.now()}`;
    const formattedGr = {
      id,
      titleMr: gr.titleMr,
      titleEn: gr.titleEn || gr.titleMr,
      number: gr.number,
      date: gr.date || new Date().toLocaleDateString('en-GB'),
    ...
    if (db) {
      await ensureGrTable(db);
      await db.prepare(`
        INSERT INTO government_gr (
          id, title_mr, title_en, number, date, dept_mr, dept_en, summary_mr, summary_en, pdf_path, status, badge_color, code_number, raw_json, created_at
      ...
      `).bind(
        ...
        formattedGr.deptMr,
        formattedGr.deptEn,
        formattedGr.summaryMr,
        formattedGr.summaryEn,
        formattedGr.pdfPath,
        formattedGr.status,
        formattedGr.badgeColor,
        formattedGr.codeNumber,
        JSON.stringify(formattedGr)
      ).run();
    }

    return new Response(JSON.stringify(formattedGr), {
      status: 201,
      headers: { 'Content-Type': 'application/json', 'Access-Control-Allow-Origin': '*' },
    });
  } catch (e) {
...
export async function onRequestDelete(context) {
  ...
  if (db && id) {
    try {
      await ensureGrTable(db);
      await db.prepare('DELETE FROM government_gr WHERE id = ?').bind(id).run();
    } catch (e) {}
  }

  return new Response(JSON.stringify({ success: true, message: 'Government GR deleted' }), {
    headers: { 'Content-Type': 'application/json', 'Access-Control-Allow-Origin': '*' },
  });
}
